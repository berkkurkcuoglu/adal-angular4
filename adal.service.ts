...
export class AdalService {
  ...
  private readonly maxErrorCount: number = 10;
  ...
  public init(configOptions: adal.Config): void {
    ...
    if (
      this.user.loginCached &&
      !this.user.authenticated &&
      window.self == window.top &&
      !this.isInCallbackRedirectMode
    ) {
      this.refreshLoginToken();
    } else if (
      this.user.loginCached &&
      this.user.authenticated &&
      !this.loginRefreshTimer &&
      window.self == window.top
    ) {
  ...
  public handleWindowCallback(removeHash: boolean = true): void {
    ...
    if (this.context.isCallback(hash)) {
      ...
      if (
        this.context._openedWindows.length > 0 &&
        ...
        this.context = this.context._openedWindows[
          this.context._openedWindows.length - 1
      ...
      var callback =
        this.context._callBackMappedToRenewStates[requestInfo.stateResponse] || this.context.callback;
  ...
  private refreshLoginToken(): void {
    ...
    this.acquireToken(<any>this.context.config.loginResource).subscribe(
      (token: string) => {
        this.user.token = token;
  ...
  private get isInCallbackRedirectMode(): boolean {
    return (
      window.location.href.indexOf('#access_token') !== -1 || window.location.href.indexOf('#id_token') !== -1
    );
  }

  private setupLoginTokenRefreshTimer(): void {
    // Get expiration of login token
    let exp = this.context._getItem(
      this.context.CONSTANTS.STORAGE.EXPIRATION_KEY + <any>this.context.config.loginResource
    );

    // Either wait until the refresh window is valid or refresh in 1 second (measured in seconds)
    let timerDelay =
      exp - this.now() - (this.context.config.expireOffsetSeconds || 300) > 0
        ? exp - this.now() - (this.context.config.expireOffsetSeconds || 300)
        : 1;
